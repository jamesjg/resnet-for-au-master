...
def train_one_epoch_cls(train_loader, model, optimizer, criterion, lr_scheduler, epoch,  args, logger):
    batch_time = AverageMeter()  #一个batch中模型运行时间
    data_time = AverageMeter()  #一个batch中加载数据时间
    loss = AverageMeter()
    mae = AverageMeter()
    ...
    model.train()
    batch_start_time = time.time()
    idx_tensor = [idx for idx in range(10)]
    ...
    for idx, (imgs, labels) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        au_preds = model(imgs)
        output = torch.zeros([bs, 24]).cuda()
        loss_cls = torch.zeros([1]).cuda()
        num_bins = 10
        quantized_labels = torch.floor(labels * num_bins).long()
        quantized_labels = torch.clamp(quantized_labels, min=0, max=num_bins - 1)
        
        for i, au_pred in enumerate(au_preds):
            loss_cls +=  nn.CrossEntropyLoss()(au_pred, quantized_labels[:,i])
            output[:,i] = torch.sum(nn.Softmax(dim=1)(au_pred)*idx_tensor, 1) * 0.1
        loss_reg = criterion(100*output, 100*labels) #这个batch的平均损失, 每个样本在每个AU上的平均均方损失
        
        loss_train = loss_reg * 0.001 + loss_cls / len(au_preds)
        loss_train.backward()
        optimizer.step()
        lr_scheduler.step_update((epoch * len(train_loader) + idx))
        lr = optimizer.param_groups[0]['lr']
...
def evalutate_cls(val_loader, model, criterion, epoch, args, logger):
    batch_time = AverageMeter()  #一个batch中模型运行时间
    data_time = AverageMeter()  #一个batch中加载数据时间
    loss = AverageMeter()
    mae = AverageMeter()
    ...
    with torch.no_grad():
        for idx, (imgs, labels) in enumerate(val_loader):
            ...
            output = torch.zeros([bs, 24]).cuda()
            for i, au_pred in enumerate(au_preds):
                #loss_cls +=  nn.CrossEntropyLoss()(au_pred, labels[:,i])
                output[:,i] = torch.sum(nn.Softmax(dim=1)(au_pred)*idx_tensor, 1) * 0.1
...
def inference(imgs, model, args, logger):
    batch_time = AverageMeter()  #一个batch中模型运行时间
    data_time = AverageMeter()  #一个batch中加载数据时间
    batch_start_time = time.time()
    print("inferencing...")
    ...
    #img_names = []
    with torch.no_grad():
        for batch_idx in range(0, len(imgs), args.batch_size):
            
            batch_imgs = imgs[batch_idx:batch_idx + args.batch_size]
            batch_imgs = torch.from_numpy(np.stack(batch_imgs)).cuda()
            if batch_idx == 0:
                ori_imgs = UnNormalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])(batch_imgs)
                torchvision.utils.save_image(ori_imgs[0],'images/inference_image_{:}.jpg'.format(batch_idx),normalize=True)

...
def build_scheduler(config, optimizer, n_iter_per_epoch):
    ...
    if config.lr_scheduler == 'cosine':
        lr_scheduler = CosineLRScheduler(
            ...
            #t_mul=1.,
            lr_min=config.min_lr, #5e-6,#config.TRAIN.MIN_LR,
            warmup_lr_init=config.warmup_lr, #5e-7,#config.TRAIN.WARMUP_LR,
            warmup_t=warmup_steps,
            cycle_limit=1,
            t_in_epochs=False,
            warmup_prefix=True, #config.TRAIN.LR_SCHEDULER.WARMUP_PREFIX,
        )
        ...
        lr_scheduler = StepLRScheduler(
            optimizer,
            decay_t=decay_steps,
            decay_rate=0.1, #config.TRAIN.LR_SCHEDULER.DECAY_RATE,
            warmup_lr_init=config.warmup_lr,
            warmup_t=warmup_steps,
            t_in_epochs=False,
        )
